...
describe('Predictor', () => {
    ...
    test('cool-off resolves the active prediction so it can re-fire', () => {
        ...
        // Cool-off threshold = 85 * 0.85 = 72.25; cpu=10 must resolve the existing prediction
    ...
    test('re-fire: a new prediction is created after cool-off when trend rises again', () => {
        // Insert 20 fresh rising samples so the regression window is clean
        ...
        predictor.runCycle({ cpu: 79, mem: 10, disk: 10 });
        const active = predictionsDAL.findActive().filter(p => p.metric === 'CPU' && p.kind === 'trend');
